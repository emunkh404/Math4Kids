const generateRandomNumbers = (type, level, score) => {
  ...
  function generateAdditionNumbers(level, score) {
    ...
    for (let i = 0; i < 10; i++) {
      const var1 = Math.floor(Math.random() * (maxNumber - minNumber + 1)) + minNumber;
      const var2 = Math.floor(Math.random() * (maxNumber - minNumber + 1)) + minNumber;
      result.push({
        var1,
        var2,
        answer: var1 + var2,
  ...
  const score = 30; // Replace with the actual score from DB
const level = "fourth";
